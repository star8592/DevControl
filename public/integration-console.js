(() => {
  ...
  function aiProviderLabel(key) {
    return { codex: 'Codex CLI', localModel: 'Local Model', openai: 'OpenAI API' }[key] || key;
  }
  ...
  function renderProviderOptions(action) {
    ...
    select.innerHTML = choices.map(item => {
      ...
      return `<option value="${esc(item.key)}" ${item.enabled ? '' : 'disabled'}>${esc(aiProviderLabel(item.key))}${suffix}</option>`;
    }).join('');
    let preferred = enabled.find(item => item.key === state.provider) || enabled.find(item => item.key === 'codex') || enabled[0] || null;
    if (preferred) {
      state.provider = preferred.key;
      select.value = preferred.key;
    }
    el('providerHint').textContent = preferred ? providerContextDescription(preferred.key) : '没有已启用且支持此动作的 AI Provider。';
  }
  ...
  function showProviderSetup(provider, message) {
    ...
    const lines = [message, '', `${aiProviderLabel(provider)} registry: ${configured}`, ''];
    if (provider === 'codex') {
      lines.push(
        '启用方式（本机，不修改 Git 仓库）：',
  ...
  async function runCopilot() {
    ...
    if (!providerConfig?.enabled) {
      showProviderSetup(provider, `${aiProviderLabel(provider)} 当前未启用。`);
      return;
    }
    if (!(providerConfig.capabilities || []).includes(state.action)) {
      setOutput(`${aiProviderLabel(provider)} 不允许 ${state.action}。`, 'warn');
      return;
    }

    const button = el('copilotRun');
    button.disabled = true;
    setOutput(`正在调用 ${aiProviderLabel(provider)} · ${state.action}…\n${providerContextDescription(provider)}`);
    try {
      ...
      if (data.unavailable) {
        showProviderSetup(provider, `${aiProviderLabel(provider)} 不可用：${data.error || 'not found'}`);
        return;
      }
      if (data.ok === false) {
        setOutput(`${data.error || 'AI 执行失败'}\n\n${data.stderr || ''}\nrequest ${data.requestId || '—'}`, 'bad');
        return;
      }
      const usage = data.usage?.totalTokens != null
        ...
        : '';
      setOutput(`${data.stdout?.trim() || 'AI 未返回文本'}\n\nprovider ${aiProviderLabel(provider)} · context ${data.contextPolicy || providerContextDescription(provider)}${usage}\nrequest ${data.requestId || '—'}`);
    } catch (error) {
      if (error.status === 403 && /Integration disabled:/.test(error.message)) {
        showProviderSetup(provider, `${aiProviderLabel(provider)} adapter 存在，但本机配置当前关闭。`);
      } else {
  ...
  async function probeProvider(project, provider = 'codex') {
    state.project = project;
    renderHeader(project, `${aiProviderLabel(provider)} 探测`, '只执行 provider health/probe，不执行项目写操作。');
    el('copilotComposer').style.display = 'none';
    openDrawer();
    setOutput(`正在探测 ${aiProviderLabel(provider)}…`);
    try {
      ...
      if (data.ok) {
        ...
        setOutput(`${details || `${aiProviderLabel(provider)} 已发现`}\n\nrequest ${data.requestId || '—'}`);
      } else {
        showProviderSetup(provider, `${aiProviderLabel(provider)} 探测失败：${data.error || 'not ready'}`);
      }
    } catch (error) {
      setOutput(`${aiProviderLabel(provider)} 探测失败：${error.message}`, 'bad');
    }
  }

  function providerChanged() {
    state.provider = el('copilotProvider')?.value || state.provider;
    el('providerHint').textContent = providerContextDescription(state.provider);
  }
