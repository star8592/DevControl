...
test('local control state joins qualification repair PR review and cleanup state', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'devcontrol-state-'));
  const state = path.join(root, 'state');
  for (const dir of [
    'latest', 'ai-queue', 'repair-plans', 'repair-attempts',
    'repair-candidates', 'repair-reviews', 'repair-cleanups'
  ]) {
  ...
  await writeFile(path.join(state, 'discovery.json'), JSON.stringify({
    ...
    projects: [{
      key: 'demo', name: 'Demo', repo: 'star8592/demo', localPath: '/tmp/demo',
      branch: 'main', sha: 'abc', dirty: false, stacks: ['node'], confidence: 1,
  ...
  await writeFile(path.join(state, 'repair-plans', 'sig.json'), JSON.stringify({
    signature: 'sig', project: { key: 'demo', repo: 'star8592/demo', sha: 'old' },
    runId: 'r0', generatedAt: '2026-01-01T00:01:00Z', mode: 'PLAN_ONLY',
    provider: 'heuristic', confidence: 0.85,
    summary: 'Reproduce the failing test and repair the smallest regression.',
    steps: ['Reproduce npm test'], acceptance: ['npm test passes']
  }));
  await writeFile(path.join(state, 'repair-attempts', 'sig.json'), JSON.stringify({
    schemaVersion: 1, project: 'demo', repo: 'star8592/demo', signature: 'sig',
    failureSha: 'old', attemptedAt: '2026-01-01T00:02:00Z', status: 'CANDIDATE_READY',
    branch: 'devcontrol/repair/demo-sig', worktree: '/tmp/worktree',
    repairSha: 'repairsha', changedPaths: ['src/app.js'], pushed: true
  }));
  await writeFile(path.join(state, 'repair-candidates', 'sig.json'), JSON.stringify({
    schemaVersion: 1, project: 'demo', repo: 'star8592/demo', signature: 'sig',
    recordedAt: '2026-01-01T00:03:00Z', status: 'PR_READY',
    branch: 'devcontrol/repair/demo-sig', repairSha: 'repairsha', pushed: true,
    pushEnabled: true, prEnabled: true,
  ...
  await writeFile(path.join(state, 'repair-reviews', 'sig.json'), JSON.stringify({
    schemaVersion: 1, project: 'demo', repo: 'star8592/demo', signature: 'sig',
    candidateStatus: 'PR_READY', reviewedAt: '2026-01-01T00:04:00Z', state: 'MERGED',
    reasons: [], headSha: 'repairsha', headRef: 'devcontrol/repair/demo-sig',
    mergeableState: 'clean', checks: { total: 2, passed: 2, pending: 0, failed: 0 },
    pr: { number: 42, url: 'https://github.com/star8592/demo/pull/42' },
    mergedAt: '2026-01-01T00:04:00Z', mergeCommitSha: 'abc', draft: false
  }));
  await writeFile(path.join(state, 'repair-cleanups', 'sig.json'), JSON.stringify({
    schemaVersion: 1, project: 'demo', repo: 'star8592/demo', signature: 'sig',
    reviewedState: 'MERGED', mergeCommitSha: 'abc', repairSha: 'repairsha',
    repairBranch: 'devcontrol/repair/demo-sig', checkedAt: '2026-01-01T00:05:00Z',
    cleanedAt: '2026-01-01T00:05:00Z', status: 'CLEANED', remoteBranchDeleted: false
  }));
  ...
  assert.equal(result.projects.demo.latestRepairReview.state, 'MERGED');
  assert.equal(result.repairCleanups.count, 1);
  assert.equal(result.projects.demo.latestRepairCleanup.status, 'CLEANED');
  assert.equal(result.projects.demo.latestRepairCleanup.remoteBranchDeleted, false);
});
