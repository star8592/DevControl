...
export async function persistQualificationResult({ result, stateDir, appendEvent = true }) {
  if (!result?.runDir || !result?.runId || !result?.project?.key) {
    throw new Error('qualification result is missing runDir/runId/project key');
  }
  ...
  await writeFile(
    path.resolve(stateDir, 'latest', `${result.project.key}.json`),
    `${JSON.stringify(result, null, 2)}\n`,
    'utf8'
  );
  if (appendEvent) {
...
export async function executeProjectQualification({
  ...
  if (persist) await persistQualificationResult({ result, stateDir });
  return result;
}
