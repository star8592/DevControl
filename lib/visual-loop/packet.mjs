...
function isGodotProductionOrigin(value) {
  return /^godot-production(?:-[a-z0-9_-]+)*$/i.test(String(value || '').trim());
}
...
export async function buildJudgePacket({
  ...
  for (const role of normalizedContract.requiredRoles) {
    ...
    assert(isGodotProductionOrigin(capture.origin), `current capture is not an allowed Godot production origin: ${role}`);
    ...
    assert(capture.blender_preview !== true, `Blender preview rejected: ${role}`);
    assert(capture.fast_scaffold !== true, `FAST scaffold rejected: ${role}`);
  ...
  const normalizedPreviousVerdict = previousVerdict
    ? validateJudgeVerdict(previousVerdict, { requiredRoles: normalizedContract.requiredRoles, weights })
    : null;
  ...
  return {
    ...
    round: {
      ...
      runnerIdentity: manifest.runner_identity || manifest.runner || null,
