...
export function validateJudgeVerdict(verdict, {
  ...
  for (const [role, view] of Object.entries(verdict.views)) {
    ...
    assert(Object.keys(normalizedScores).length > 0, `views.${role} has no recognized scores`);
  ...
  const normalizedBlockers = blockers.map((blocker, index) => {
    ...
    assert(dimension in weights, `blockers[${index}].dimension is unknown`);
...
export function buildJudgeInstruction({ passScore = 8.5, requiredRoles = [] } = {}) {
  return [
    ...
    'Be strict about camera/composition, human/world scale, character readability, architecture silhouette and construction, materials, lighting, fine detail, and motion where applicable.',
    ...
    'For each view: { scores: {composition, scale, character, architecture, materials, lighting, detail, motion as applicable}, summary }.',
