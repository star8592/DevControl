...
async function readRepairPlans(stateDir) {
  ...
  const plans = [];
  let names = [];
  try { names = await readdir(dir); } catch { return plans; }
...
async function readRepairAttempts(stateDir) {
  ...
  const attempts = [];
  let names = [];
  try { names = await readdir(dir); } catch { return attempts; }
  for (const name of names.filter(name => name.endsWith('.json')).sort()) {
    const value = await readJson(path.join(dir, name));
...
async function readRepairCandidates(stateDir) {
  ...
  const candidates = [];
  let names = [];
  try { names = await readdir(dir); } catch { return candidates; }
  for (const name of names.filter(name => name.endsWith('.json')).sort()) {
    const value = await readJson(path.join(dir, name));
    if (!value?.project) continue;
    candidates.push({
...
async function readRepairReviews(stateDir) {
  const dir = path.resolve(stateDir, 'repair-reviews');
  const reviews = [];
  let names = [];
  try { names = await readdir(dir); } catch { return reviews; }
  for (const name of names.filter(name => name.endsWith('.json')).sort()) {
    const value = await readJson(path.join(dir, name));
    if (!value?.project) continue;
    reviews.push({
...
async function readFailureQueue(stateDir) {
  ...
  const files = [];
  let names = [];
  try { names = await readdir(queueDir); } catch { return files; }
  for (const name of names.filter(name => name.endsWith('.json')).sort()) {
