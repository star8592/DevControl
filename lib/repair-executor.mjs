...
import { executeQualificationPipeline } from './qualification-pipeline.mjs';
import { inspectProjectGit } from './project-lifecycle.mjs';
import { visualQualificationEnabled } from './visual-qualification.mjs';
...
export function assessAutoFixEligibility({ project, packet, plan }) {
  ...
  const failedCommand = String(packet?.failure?.failedCommand || '');
  const visualFailure = failedCommand.startsWith('visual:');
  if (!safeCommands.includes(failedCommand) && !(visualFailure && visualQualificationEnabled(project))) {
    reasons.push('failed gate is outside the project safe qualification/visual allowlist');
  }
...
function repairPrompt(packet, plan) {
  const visual = packet.failure?.visual || null;
  return [
    ...
    `Failed gate: ${packet.failure?.failedCommand || '-'}`,
    `Failure categories: ${(packet.failure?.categories || []).join(', ')}`,
    visual ? `Visual failure reasons: ${(visual.failureReasons || []).join(' | ')}` : '',
    visual ? `Visual evidence directory: ${visual.outputDir || '-'}` : '',
    '',
    'Repair plan:',
    plan.summary || '',
...
export async function executeAutoFix({
  ...
  const failedCommand = String(packet.failure.failedCommand || '');
  const visualFailure = failedCommand.startsWith('visual:');
  const commands = visualFailure
    ? [...safeCommands]
    : [failedCommand, ...safeCommands.filter(command => command !== failedCommand)];
  ...
  const validation = await executeQualificationPipeline({
  ...
  return {
    ...
    changedPaths: paths,
    validation,
    visualValidation: validation.visualQualification || null,
