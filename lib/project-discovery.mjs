...
async function findGodotProjectPaths(root, maxDepth = 2) {
  const found = [];
  async function walk(current, depth) {
    if (await exists(path.join(current, 'project.godot'))) {
    ...
    for (const entry of entries) {
      ...
      if (DEFAULT_SKIP.has(entry.name) || entry.name.startsWith('.')) continue;
...
async function inferProject(root) {
  ...
  const godotProjectPaths = await findGodotProjectPaths(root, 2);
  if (godotProjectPaths.length) {
    stacks.push('godot');
    for (const godotPath of godotProjectPaths) {
      evidence.push(godotPath === '.' ? 'project.godot' : `${godotPath}/project.godot`);
    }
    if (godotProjectPaths.length === 1) {
      const godotPath = godotProjectPaths[0];
      addProposal(
        proposals,
        `godot --headless --path ${quotedArg(godotPath)} --quit`,
        'Godot project detected; parse/start smoke only',
        0.86
      );
    } else {
      riskFlags.push(`multiple Godot project roots detected: ${godotProjectPaths.join(', ')}; visual root must be explicit`);
    }
  }
  ...
  const confidenceParts = [
    repo ? 0.35 : 0,
    markers.size || godotProjectPaths.length ? 0.25 : 0,
...
function matchRegistration(project, configuredProjects = []) {
  ...
  return {
    ...
    conflicts,
    visualQualification: match.visualQualification || null
  };
}
...
export { findGitProjects, inferProject, parseGithubRemote, matchRegistration, findGodotProjectPaths };
